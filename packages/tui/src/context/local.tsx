...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
      ...
      return {
        ...
        set(name: string) {
          if (!agents().some((x) => x.name === name))
            return toast.show({
              variant: "warning",
              message: t("Agent not found: {name}", { name }),
              duration: 3000,
            })
    ...
    function createModel() {
      ...
      return {
        ...
        parsed: createMemo(() => {
          ...
          if (!value) {
            return {
              provider: t("Connect a provider"),
              model: t("No provider selected"),
              reasoning: false,
            }
          }
        ...
        cycleFavorite(direction: 1 | -1) {
          ...
          if (!favorites.length) {
            toast.show({
              variant: "info",
              message: t("Add a favorite model to use this shortcut"),
              duration: 3000,
            })
            return
          }
        ...
        set(model: { providerID: string; modelID: string }, options?: { recent?: boolean }) {
          batch(() => {
            if (!isModelValid(model)) {
              toast.show({
                message: t("Model {providerID}/{modelID} is not valid", { providerID: model.providerID, modelID: model.modelID }),
                variant: "warning",
                duration: 3000,
              })
              return
    ...
    createEffect(() => {
      ...
      toast.show({
        variant: "warning",
        message: t("Agent {name}'s configured model {providerID}/{modelID} is not valid", { name: value.name, providerID: value.model.providerID, modelID: value.model.modelID }),
        duration: 3000,
      })
    })
