...
export function Prompt(props: PromptProps) {
  ...
  const placeholderText = createMemo(() => {
    ...
    if (store.mode === "shell") {
      ...
      return t("Run a command... ") + `"${example}"`
    }
                  ...
                  {(agent) => (
                    <>
                      <text fg={fadeColor(highlight(), agentMetaAlpha())}>
                        {store.mode === "shell" ? t("Shell") : Locale.titlecase(agent().name)}
                      </text>
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          <Switch>
              ...
              >
                    ...
                    {(() => {
                      ...
                      const message = createMemo(() => {
                        ...
                        if (r.message.includes("exceeded your current quota") && r.message.includes("gemini"))
                          return t("gemini is way too hot right now")
                      ...
                      const retryText = () => {
                        ...
                        const retryInfo = t(" [retrying ") + (duration ? t("in ") + duration + " " : "") + t("attempt #") + String(r.attempt) + "]"
                ...
                <text fg={store.interrupt > 0 ? theme.primary : theme.text}>
                  ...
                  <span style={{ fg: store.interrupt > 0 ? theme.primary : theme.textMuted }}>
                    {store.interrupt > 0 ? t("again to interrupt") : t("interrupt")}
                  </span>
                </text>
              </box>
            </Match>
                    ...
                    {(() => {
                      ...
                      if (item.type === "new") {
                        if (workspace.creating())
                          return t("Creating ") + item.workspaceType + ".".repeat(workspace.creatingDots())
                        return (
                          <>
                            {t("Workspace")} <span style={{ fg: theme.textMuted }}>({t("new")} {item.workspaceType})</span>
                          </>
                        )
                      }
                      return (
            ...
            <Match when={move.pendingNew()}>
              <box paddingLeft={3}>
                <text fg={theme.accent}>{t("(new working copy)")}</text>
              </box>
            </Match>
