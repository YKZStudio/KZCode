...
export function warpReminderText(dir: string) {
  return t('<system-reminder>The user has changed the current working directory to "{dir}". This is still the same project but at a possibly new location; take this into account when working with any files from now on.</system-reminder>', { dir })
}
...
export async function warpWorkspaceSession(input: {
  ...
  if (!result?.data) {
    ...
    input.toast.show({
      title: t("Failed to warp session"),
      message: errorMessage(result?.error ?? t("no response")),
      variant: "error",
    })
    return false
  }
