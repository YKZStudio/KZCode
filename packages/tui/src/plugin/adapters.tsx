...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    slots: {
      register() {
        throw new Error(t("slots.register is only available in plugin context"))
      },
    },
    plugins: {
      list() {
      ...
      async install() {
        return {
          ok: false,
          message: t("plugins.install is only available in plugin context"),
        }
      },
    },
    theme: {
      ...
      async install(_jsonPath) {
        throw new Error(t("theme.install is only available in plugin context"))
      },
      mode() {
