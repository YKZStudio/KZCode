...
export const TuiLocale = Schema.Literals(["en", "zh", "zht"])
export type TuiLocale = Schema.Schema.Type<typeof TuiLocale>
...
export const Info = Schema.Struct({
  ...
  locale: Schema.optional(TuiLocale).annotate({ description: "UI language: en, zh (简体中文), or zht (繁體中文)" }),
})
export type Info = Schema.Schema.Type<typeof Info>

export type Resolved = Omit<Info, "attention" | "keybinds" | "leader_timeout" | "mouse" | "locale"> & {
  attention: {
    enabled: boolean
...
function detectSystemLocale(): TuiLocale {
  const lang = process.env.LC_ALL || process.env.LC_MESSAGES || process.env.LANG || ""
  const normalized = lang.toLowerCase()
  if (normalized.startsWith("zh")) {
    if (normalized.includes("hant") || normalized.includes("tw") || normalized.includes("hk")) return "zht"
    return "zh"
  }
  return "en"
}
...
export function resolve(input: Info, options: ResolveOptions): Resolved {
  ...
  return {
    ...
    locale: input.locale ?? detectSystemLocale(),
  }
}
