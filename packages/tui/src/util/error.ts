...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    const suggestions = Array.isArray(model.suggestions)
      ...
      : []
    return [
      t("Model not found: {model}", { model: `${field(model, "providerID")}/${field(model, "modelID")}` }),
      ...(suggestions.length ? [t("Did you mean: {suggestion}", { suggestion: suggestions.join(", ") })] : []),
  ...
  if (provider)
    return t('Failed to initialize provider "{provider}". Check credentials and configuration.', { provider: field(provider, "providerID") })

  const json = configData(input, "ConfigJsonError")
  if (json) {
    const path = field(json, "path")
    const message = field(json, "message")
    const base = t("Config file at {path} is not valid JSON(C)", { path: path ?? "" })
    return base + (message ? `: ${message}` : "")
  }

  const directory = configData(input, "ConfigDirectoryTypoError")
  if (directory) {
    return t('Directory "{dir}" in {path} is not valid. Rename the directory to "{suggestion}" or remove it. This is a common typo.', { dir: field(directory, "dir"), path: field(directory, "path"), suggestion: field(directory, "suggestion") })
  }
  ...
  if (invalid) {
    ...
    const issues = Array.isArray(invalid.issues)
      ...
      : []
    const prefix = t("Configuration is invalid")
    const suffix = path && path !== "config" ? ` ${t("at {path}", { path })}` : ""
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return t('MCP server "{name}" failed. Note, opencode does not support MCP authentication yet.', { name: name ?? "" })
  }
  return undefined
}
  ...
  if (typeof error === "object" && error !== null) {
    try {
      ...
      if (json === "{}") {
        ...
        return names.length === 0 ? t("{prefix} (no message)", { prefix }) : `${prefix} { ${names.join(", ")} }`
      }
      return json
    } catch {
      return t("Unexpected error (unserializable)")
    }
  }
